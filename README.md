...
> Note: Render's free tier spins down after 15 minutes of inactivity. Use [UptimeRobot](https://uptimerobot.com) to ping your URL every 5 minutes and prevent cold starts during demos.

### MongoDB Atlas — IP Whitelist

Go to Atlas → Network Access → Add IP Address → Allow Access from Anywhere (`0.0.0.0/0`).  
This is required so Render's dynamic IPs can connect to Atlas.

---

## Frontend

The frontend (React + Vite) connects to this API.  
See `API_DOCUMENTATION.md` for full request/response documentation.  
See `data.js` for mock data matching the exact API response structure — useful for frontend development without a running backend.
